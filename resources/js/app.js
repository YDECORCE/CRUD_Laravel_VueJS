...
const app = new Vue({
    ...
    methods: {
        ...
        var model = document.getElementById('myModal').value;
            axios.post('/edit/' + id_val_1.value, {val_1: make_val_1.value, val_2: model_val_1.value})
            .then(response => {
                _this.getCars();
                _this.setVal(id_val_1.value, make_val_1.value, model_val_1.value);
            });
        }, 
        ...
        deleteCar: function deleteCar(car) {
            ...
            var _this = this;
            axios.post('/delete/' + car.id).then(function(response){
             _this.getCars();
                }).catch(error=>{
